...
var init_mcpFireCore = __esm({
  "utils/mcpFireCore.ts"() {
    "use strict";
    DEFAULT_MAX_TOOL_NAME_LEN = 64;
    MCP_FIRE_NAME_PREFIX = "mcp__";
    MCP_FIRE_NAME_BUDGET = DEFAULT_MAX_TOOL_NAME_LEN - MCP_FIRE_NAME_PREFIX.length;
    sanitizeMcpToolName = (name, maxLen = DEFAULT_MAX_TOOL_NAME_LEN) => {
      const len = Math.max(1, maxLen);
      return (name || "tool").replace(/[^A-Za-z0-9_-]/g, "_").slice(0, len);
    };
    withMcpDedupeSuffix = (base, i, maxLen = DEFAULT_MAX_TOOL_NAME_LEN) => {
      const suffix = `_${i}`;
      return base.slice(0, Math.max(0, maxLen - suffix.length)) + suffix;
    };
    serverSlug = (server, maxLen = DEFAULT_MAX_TOOL_NAME_LEN) => sanitizeMcpToolName(server.name, maxLen).slice(0, 20);
    buildMcpNameMap = (servers, opts = {}) => {
    ...
    MCP_RESULT_MAX_CHARS = 2e4;
    formatMcpToolResult = (data) => {
      let s;
      try {
    ...
    stripTextFakedMcpCalls = (content, calls) => {
      let cleaned = content;
      for (const call of calls) cleaned = cleaned.split(call.matched).join("");
      return cleaned.replace(/\n{3,}/g, "\n\n").trim();
    };
    escapeRegExp2 = (s) => s.replace(/[.*+?^${}()|[\]\\/]/g, "\\$&");
    stripQuotes = (s) => {
      const t = s.trim();
      const m = t.match(/^(['"`「『])([\s\S]*)(['"`」』])$/);
      return m ? m[2] : t;
    };
    positionalKeys = (schema) => {
    ...
    coerceBySchema = (value, schema, key) => {
    ...
    extractTextFakedMcpCalls = (content, resolve, opts = {}) => {
    ...
    MCP_SUPPORTED_HANDSHAKE_PROTOCOL_VERSIONS = [
      "2025-11-25",
      "2025-06-18",
      "2025-03-26"
    ];
    MCP_REQUEST_TIMEOUT_MS = 6e4;
    ...
    buildRpcRequest = (session, method, params, isNotification = false) => {
    ...
    postCore = async (target, session, body, timeoutMs, expectResponse = true) => {
    ...
    initializeCore = async (target, session, timeoutMs) => {
    ...
    ensureInitializedCore = async (target, session, timeoutMs) => {
    ...
    normalizeMcpValueBySchema = (value, rawSchema, rootSchema, depth) => {
    ...
    normalizeMcpToolArguments = (args, inputSchema) => normalizeMcpValueBySchema(args, inputSchema, inputSchema, 0);
    targetHost = (url) => {
      try {
    ...
    callMcpToolCore = async (target, session, toolName, args = {}, opts = {}) => {
    ...
    buildMcpDirectHeaders = (server, sessionId, protocolVersion = null) => {
      const headers = {
    ...
    filterMcpServersForChar = (servers, charId) => (servers || []).filter(
      (s) => !!s.url && (s.tools?.length || 0) > 0 && (!s.charIds?.length || s.charIds.includes(charId))
    );
    buildMcpFireTools = (resolve) => {
    ...
    buildMcpFireBlock = (resolve, opts) => {
...
var reconcileSelfLogWithPack = (stored, pack, lastUserMessageAt) => {
  ...
  if (lastUserMessageAt != null && (log2.anchorUserMsgAt == null || lastUserMessageAt > log2.anchorUserMsgAt)) {
    log2 = { ...log2, anchorUserMsgAt: lastUserMessageAt, entries: [], unansweredSends: 0, recurringSends: {} };
  }
...
var countRecurringSends = (log2, clientTaskId) => clientTaskId && log2?.recurringSends?.[clientTaskId] || 0;
var bumpRecurringSend = (log2, clientTaskId) => {
  if (!clientTaskId) return log2;
  const counts = log2.recurringSends ?? {};
  return { ...log2, recurringSends: { ...counts, [clientTaskId]: (counts[clientTaskId] ?? 0) + 1 } };
};
...
function redactDevDebugSecrets(value) {
  ...
  if (typeof value === "string" && /^data:image\/[a-z0-9.+-]+;base64,/i.test(value)) {
    return `<image data omitted \xB7 ${value.length} chars>`;
  }
...
// utils/apiCallLog.ts
var API_REQUEST_CAPTURE_EVENT = "sully-api-request-capture-change";
var API_REQUEST_CAPTURE_ARMED_KEY = "sully_api_request_capture_armed_v1";
function emitCaptureChange(status) {
  try {
...
function readApiRequestCaptureArmed() {
  try {
...
var apiRequestCaptureArmed = readApiRequestCaptureArmed();
try {
  if (typeof window !== "undefined") {
...
var isQuietHour = (hour, [start, end]) => start === end ? false : start < end ? hour >= start && hour < end : hour >= start || hour < end;
var decideNaturalProactive = (input) => {
  ...
  if (recentSendMinutes < 30) {
    score -= 0.42;
    reasons.push("\u521A\u4E3B\u52A8\u8054\u7CFB\u8FC7");
  } else if (recentSendMinutes < 90) {
    score -= 0.22;
